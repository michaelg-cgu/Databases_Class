...
CREATE TABLE MG_Employee_Table(
	...
	ALTER TABLE MG_Employee_Table ADD CONSTRAINT LoginID_Constraint UNIQUE (LoginID);
	ALTER TABLE MG_Employee_Table ADD CONSTRAINT Gender_Constraint CHECK (Gender in ('M', 'F'));


CREATE TABLE MG_Employee_Pay_History(
	BusinessEntityID INT NOT Null,
	...
	CONSTRAINT FK_BusinessEntityID FOREIGN KEY (BusinessEntityID)
		REFERENCES MG_Employee_Table (BusinessEntityID)
	)

	INSERT INTO MG_Employee_Pay_History
	...
	select *
	From MG_Employee_Table

	Insert into MG_Employee_Table (BusinessEntityID)
	Values (1);

	Insert into MG_Employee_Table (BusinessEntityID, NationalIDNumber, LoginID, OrganizationNode, JobTitle, BirthDate, MaritalStatus,
	...
	Delete from MG_Employee_Pay_History
	Where BusinessEntityID = 1


	Update MG_Employee_Table
	Set NationalIDNumber = null
	Where BusinessEntityID = '1'

	Insert into MG_Employee_Table (BusinessEntityID, NationalIDNumber, LoginID)
	Values ('292', '134219713', 'adventure-works\ken0');

delete from MG_Employee_Table
where BusinessEntityID = 1;
...
CREATE TABLE MG_Products(
	...
	CREATE CLUSTERED INDEX MGP_Indexx ON MG_Products(PROD_ID)
	WITH (DROP_EXISTING = ON, FILLFACTOR  = 90);
	CREATE INDEX MG_Non_Clustered ON MG_Products(PROD_CATEGORY)
	WITH (DROP_EXISTING = ON, FILLFACTOR = 90);
...
ON IST303_MikeG_DB.dbo.MG_Employee_Table
AFTER INSERT
AS
if Not Exists(
Select * From MG_Employee_Table
)
Begin
...
Else
Begin
   PRINT 'Rows inserted from MG_Employee_Table1'
END
GO
...
Else
Begin
   PRINT 'Rows Deleted from MG_Employee_Table'
END
GO
...
Else
Begin
   PRINT 'Rows updated from MG_Employee_Table'
END
GO
...
Create Trigger After_Insert ON IST303_MikeG_DB.dbo.MG_Employee_Table
For Insert
AS Declare
...
PRINT 'Rows inserted from MG_Employee_Table'
Go

DROP TRIGGER  IF EXISTS  After_Insert


Alter Trigger After_Delete ON IST303_MikeG_DB.dbo.MG_Employee_Table
For Delete
AS Declare
    @BusinessEntityID INT,
	@NationalIDNumber nvarchar(15),
...
Alter TRIGGER AfterUpdate ON IST303_MikeG_DB.dbo.MG_Employee_Table
FOR UPDATE
AS
...
Select @ModifiedDate = i.ModifiedDate From Inserted i;
	...
	if update(NationalIDNumber)
		set @audit_action='Updated Row from MG_Employee_Table.';
	if update(LoginID)
		set @audit_action='Updated Row from MG_Employee_Table.';
...
PRINT 'Updated Row from MG_Employee_Table.'
GO

---Creating and dropping indexes and showing text plans
elect *
Into IST303_MikeG_DB.dbo.MG_Promotions From IST302.dbo.PROMOTIONS

Select *
Into IST303_MikeG_DB.dbo.MG_Customers From IST302.dbo.CUSTOMERS

Select *
Into IST303_MikeG_DB.dbo.MG_Channels From IST302.dbo.CHANNELS

Select *
Into IST303_MikeG_DB.dbo.MG_Times From IST302.dbo.TIMES

Select *
Into IST303_MikeG_DB.dbo.MG_Products_CE4 From IST302.dbo.PRODUCTS
...
FROM MG_Sales S
INNER JOIN MG_Customers C
	ON S.CUST_ID = C.CUST_ID
INNER JOIN MG_Products_CE4 P
	ON S.PROD_ID = P.PROD_ID
INNER JOIN MG_Promotions R
	ON S.PROMO_ID = R.PROMO_ID
INNER JOIN MG_Channels H
	ON S.CHANNEL_ID = H.CHANNEL_ID
INNER JOIN MG_TIMES T
...
WHERE CUST_GENDER = 'M' AND CUST_MARITAL_STATUS = 'Married'
	...
	Create Index IDX_Sales_Cust_ID on MG_Sales (Cust_ID)
	Create Index IDX_Sales_Prod_ID on MG_Sales (Prod_ID)
	Create Index IDX_Sales_Promo_ID on MG_Sales (Promo_ID)
	Create Index IDX_Sales_Channel_ID on MG_Sales (Channel_ID)
	Create Index IDX_Sales_Time_ID on MG_Sales (Time_ID)
	Create Index IDX_Cust_ID on MG_Customers (Cust_ID)

	Create NONCLUSTERED Index NIDX_Customers_Cust_Gender on MG_Customers (Cust_Gender)
	Create NONCLUSTERED Index NIDX_Customers_Cust_Martial_Status on MG_Customers (CUST_MARITAL_STATUS)
	Create NONCLUSTERED Index NIDX_Times_FISCAL_MONTH_NAME on MG_Times (FISCAL_MONTH_NAME)
	Create NONCLUSTERED Index NIDX_Products_CE4_PROD_SUBCATEGORY on MG_Products_CE4 (PROD_SUBCATEGORY)
	Create NONCLUSTERED Index NIDX_Channels_CHANNEL_DESC on MG_Channels (CHANNEL_DESC)
	Create NONCLUSTERED Index NIDX_Time_ID on MG_Times (TIME_ID)


CREATE INDEX Composite_Gender_Martial_Status
on MG_Customers (Cust_Gender, Cust_Marital_Status);
